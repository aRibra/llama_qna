...
data_src = 'wikitext' # 'wikitext' or 'coaxnn'

if data_src == 'coaxnn':
    df = pd.read_csv("train_coaxnn_paper_text.csv")
    sample_test = Dataset.from_pandas(df)
    # Select random indices for metric evaluation
    np.random.seed(42)
    rand_ixs = np.random.randint(0, len(sample_test), len(sample_test))
    sample_test = sample_test.select(rand_ixs)
...
base_chkpnt_path = "llama2-7b-coaxnn-paper/checkpoint-"
